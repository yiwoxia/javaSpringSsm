...
<script type="text/javascript">
	...
	function delBanji(cid) {
		...
		if (isDelEle) {
			location.href="${pageContext.request.contextPath }/course?method=delCourse&id=" + cid;
		}
	}
</script>
</head>
		...
		<div class="row">
			...
			<div class="col-md-10">
				<ul class="nav nav-tabs">
					<li role="presentation" class="active"><a href="#">班级管理</a></li>
					<li role="presentation"><a href="${pageContext.request.contextPath }/course?method=toAddCourse">添加班级</a></li>
				</ul>
				<!-- 查询条件 开始 -->
				<form id="searchForm" action="${pageContext.request.contextPath}/course?method=seacherByCondition" method="post" class="form-inline" style="float: left; margin-top: 10px;margin-bottom: 10px;" >
					<!-- 用于查找+分页 记录反的页数 -->
					<input type="hidden" name="pageIndex" id="pageIndex" />
					<div class="form-group"  >
						<label for="exampleInputName2">班级：</label>
					    <input type="text" name="cname" value="${condition.name }" class="form-control" id="exampleInputName2" placeholder="姓名">
				 	</div>
